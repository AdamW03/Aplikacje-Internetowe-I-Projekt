<x-app-layout>
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <x-slot name="header">
            <h1 class="text-2xl font-semibold text-gray-800">Edit Game</h1>
        </x-slot>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form method="POST" action="{{ route('games.update', $game->id) }}" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <select name="game_tags[]" id="game_tags" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" multiple>
                        @foreach ($gameTags as $tag)
                            <option value="{{ $tag->id }}" {{ in_array($tag->id, old('game_tags', $game->game_tags ? $game->game_tags->pluck('id')->toArray() : [])) ? 'selected' : '' }}>
                                {{ $tag->name }}
                            </option>
                        @endforeach
                    </select>
